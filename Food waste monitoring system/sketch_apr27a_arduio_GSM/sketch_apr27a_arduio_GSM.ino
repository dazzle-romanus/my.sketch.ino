...
void setup() {
  ...
  // Optional: wait for GSM module to be ready
  delay(3000);
  gsm.println("AT");
  delay(1000);
}
...
void loop() {
  ...
  // Immediate SMS when spoiled and status changed
  if (isSpoiled && lastStatus != "SPOILED") {
    sendSMS("⚠️ Food Spoilage Detected!\nTemp: " + String(temperature) + "C\nHumidity: " + String(humidity) + "%\nMethane: " + String(methaneConcentration) + " ppm");
    // Optional: make emergency call (uncomment below)
    // makeCall("9876543210"); 
  }

  // Periodic SMS every 2 hours
